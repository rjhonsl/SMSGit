	...
	public void onReceive(Context context, Intent intent) {
		...
		try {
			...
			if (extras != null) {
				...
				final String finalmessage, finalNumber, finalBody;
...
//					Log.d("LOOP","loop");
				...
				if (sender_num_six.equals("+63") || sender_num_zero.equals("0")) {
					...
					if (Num.equalsIgnoreCase(finalNum) && !codeDB.isEmpty() && branchCode2.equals(branchCode)) {
						...
						//if existing on Queue --- RECEIVED TEXT SHOULD HAVE 'SNT' ON THE END //////////////
						...
						}else if(!cursor.moveToFirst() && key.equalsIgnoreCase("SNT")) {
							Log.d("SNT", "SNT BUT NOT IN QUEUE");
						}
						else{
							Log.d("CONFIRMATION", "AutoReply - new request");
...
//					showNotificationMain_unreg(contactName, "not FIltered");
	...
	public Cursor compareMessageToQueue(String message) {
		Log.d("CONFIRMATION", "compare message");
		String idinDB = "null";
